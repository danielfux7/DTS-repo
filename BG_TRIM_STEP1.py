...
if __name__ == '__main__':
    ...
    command = 'cdie.soc_cr_wrapper.' + name + '.inst0.dfvfreg7_inst.ldo1p2_vref_range_sel = 110'
    exec(command)
    ...
    command = 'cdie.taps.cdie_' + name + '.tapconfig.adcdfxextvref = 0x0'
    exec(command)
    command = 'cdie.soc_cr_wrapper.' + name + '.inst0.dfvfreg4_inst.adc_vrefldo_ext_vref_sel = 1111'
    exec(command)

    # e. Program vref_ldo output mux to select external reference as adc reference voltage
    command = 'cdie.soc_cr_wrapper.' + name + '.inst0.dfvfreg4_inst.adc_vrefldo_ext_vref_sel = 0x1'
    exec(command)

    # f. Program ADC supply buffer to select external input reference voltage
    command = 'cdie.soc_cr_wrapper.' + name + '.inst0.dfvfreg16_inst.adc_supply_buf_vref_ext_sel = 0x1'
    exec(command)
    command = 'cdie.soc_cr_wrapper.' + name + '.inst0.dfvfreg32_inst.adc_supply_buf_out_sel = 0x1'
    exec(command)
